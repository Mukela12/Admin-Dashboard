...
function transformApplication(doc: FirebaseFirestore.DocumentSnapshot) {
  ...
  return {
    ...
    insuranceCertificate: data.vehicle?.insuranceUrl || data.vehicle?.insuranceCertificateUrl || data.insuranceCertificate || '',
    ...
    vehicleReg: data.vehicle?.registration || data.vehicle?.plateNumber || data.vehicleReg || '',
    carMake: data.vehicle?.make || data.carMake || '',
    carModel: data.vehicle?.model || data.carModel || '',
    nrc: data.identity?.nrcNumber || data.identity?.nrc || data.nrc || '',
